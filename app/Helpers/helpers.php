...
if (!function_exists('PreviewStatus')) {
    ...
    {
      if ($order->previewStatus->status_code == 'pend'){
        return '<span class="label label-primary circle" style="font-size:13px">
      ...
      }elseif($order->previewStatus->status_code == 'success'){
      ...
      }elseif($order->previewStatus->status_code == 'payment_failed'){
        return '<span class="label label-danger circle" style="font-size:13px">
                  '.$order->previewStatus->name_ar.'
               </span>';
      }
